...
def monte_carlo(cur_order, iters=100000, printDist=False):
    best_cost = total_l1(cur_order)
    best_order = copy.deepcopy(cur_order)
    for _ in range(iters):
        cur_order = cur_order[np.random.permutation(cur_order.shape[0]), :]
        cur_cost = total_l1(cur_order)
...
def random_walk(cur_order, iters=100000, printDist=False):
    def neighbour_l1(i):
        answer = 0
        if i != 0:
            answer += l1(cur_order[i], cur_order[i - 1])
    ...
    for _ in range(iters):
        ...
        cur_cost -= neighbour_l1(i1) + neighbour_l1(i2)
...
def try_all_swaps(cur_order, cur_cost, n, printDist=False):
    def neighbour_l1(i):
        answer = 0
        if i != 0:
            answer += l1(cur_order[i], cur_order[i - 1])
...
def hill_climb(cur_order, iters=100000, printDist=False):
    cur_order = cur_order[np.random.permutation(cur_order.shape[0]), :]
    cur_cost = total_l1(cur_order)
    best_cost = cur_cost
    best_order = copy.deepcopy(cur_order)
    ...
    for _ in range(iters):
        best_order, best_cost = try_all_swaps(best_order, best_cost, n, printDist=printDist)
...
def annealing(cur_order, iters=100, delta_t=0.0001, t_max=100, printDist=False):
...
def genetic_algo(cur_order, iters, size=500, sel_rate=0.1, mut_rate=0.05, printDist=False):
    mut_rate = min(1.0, mut_rate)  # for protection
    best_order = copy.deepcopy(cur_order)
    best_cost = total_l1(best_order)

    world = [cur_order[np.random.permutation(cur_order.shape[0]), :].tolist() for _ in range(size)]
    n = len(cur_order)
    for i in range(iters):
        # Selection: take best 10%
        cur_relatives = sorted(world, key=total_l1)[0:int(size * sel_rate)]
        world = []
